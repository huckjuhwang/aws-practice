...
### ex08.json ( node 프로젝트를 AWS에 적용 #1 )
```bash
...
### ex09.json ( node 프로젝트를 AWS에 적용 #2 )
```bash
$ aws cloudformation create-stack --stack-name myserver --template-body https://raw.githubusercontent.com/huckjuhwang/aws-practice/main/02/ch04/04/ex09.json --parameters ParameterKey=KeyName,ParameterValue=mykey ParameterKey=VPC,ParameterValue=vpc-3ca43a57 ParameterKey=InstanceType,ParameterValue=t2.micro
